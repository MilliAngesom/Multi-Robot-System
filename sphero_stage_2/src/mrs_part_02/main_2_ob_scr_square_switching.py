...
from utils_2.boid_2_ob_scr_square import Boid
...
class Robot_move:
    ...
    def odom_callback(self, msg):
        ...
        # Checking for visible agents
        if self.agent.id==0:
            self.agent.edges=[None]
        else:
            self.agent.edges=[]
        ...
        if self.agent.id == 1: # if not a stub born
            ...
            if self.calculate_distance(agent_local_1.position_v, self.agents[0].position_v)<= self.search_radius:
                self.edges[agent_local_1.id] = [0]
    
            else: 
                self.edges[agent_local_1.id] = []
                self.send_velocity(frame_local_1, Vector2())
        ...
        if self.agent.id == 2: # if not a stub born
            ...
            if self.calculate_distance(agent_local_2.position_v, self.agents[0].position_v)<= self.search_radius:
                self.edges[agent_local_2.id] = [0, 1, 3]
        ...
        if self.agent.id == 3: # if not a stub born
            frame_local_3 = frame_id
            agent_local_3 = self.agent

            agent_vel_3 = self.update_param(msg, agent_local_3, self.agents)
            self.a_final_vel_3 = [float(agent_vel_3.x), float(agent_vel_3.y)]

            if self.a_final_vel_3[0] != 0.0 or self.a_final_vel_3[1] != 0.0:
                if self.count_3_f == 10:
                    self.count_3_f = 1
                    a_repel_vel_3 = self.repulsive_vector_fn(msg, agent_local_3, self.a_final_vel_3, self.cylinder_length, Leader=True)
                    if a_repel_vel_3.x != 0.0 or a_repel_vel_3.y != 0.0:
                        print('obstacle infront of Third follower')

                        self.repul_prev_v = a_repel_vel_3
                else: 
                    self.count_3_f += 1
            self.send_velocity(frame_local_3, agent_vel_3)
    ...
    def __init__(self):
        ...
        """ Formation """
        self.d = 0.6
        self.search_radius      = 1.0  # should a little bit more than d
        ...
        self.separation_dist[1] = [Vector2(self.d,0)]
        self.separation_dist[2] = [Vector2(self.d,-self.d), Vector2(0,-self.d), Vector2(self.d,0)]
        self.separation_dist[3] = [Vector2(0,-self.d), Vector2(-self.d,-self.d)]
        ...
        # self.separation_dist[0] = [Vector2(0,0)]
        # self.separation_dist[1] = [Vector2(0.5*d,-0.866*d)]
        # self.separation_dist[2] = [Vector2(-0.5*d,-0.866*d), Vector2(-d,0)]
        ...
        # self.separation_dist[0] = [Vector2(0,0)]
        # self.separation_dist[1] = [Vector2(0,-d)]
        # # self.separation_dist[2] = [Vector2(0,d), Vector2(0,2*d)]
        # self.separation_dist[2] = [Vector2(0,d)]
        ...
        # Tunable parameters
        # Leader
        self.max_speed_leader   = 0.5 # 0.5
        self.max_repul_speed    = self.max_speed_leader # 0.5
        self.max_speed          = 1.4*self.max_speed_leader # 0.7
        ...
        # Initialize a publisher for search_radius
        self.search_radius_pub = rospy.Publisher("search_radius", Float32, queue_size=10)
    ...
    def param_callback(self, config, level):
            ...
            self.max_speed_leader   = config.max_speed_leader

            self.max_repul_speed    = self.max_speed_leader # 0.5
            self.max_speed          = 1.4*self.max_speed_leader # 0.7

            self.search_radius_pub.publish(Float32(self.search_radius))
